...
- **Wire list**: node · capacitance · connections as `device.port`, comma-
  ...
  budget wins over uniformity. Connections that still cannot fit continue on
  continuation rows (node repeated with `↳`), split at item boundaries —
  never mid-item, never silently dropped. Ground row is always last and
  shows `(ref)`.
